...
def ABX(
    ...
    if "within" in modes:
        ...
        extras["divisor_speaker_within"] = divisor_speaker
    ...
    if "across" in modes:
        ...
        extras["divisor_speaker_across"] = divisor_speaker
...
def parse_args(argv):
    ...
    parser_checkpoint.add_argument(
        "path_dataset", type=str, help="Path to the dataset"
    )
    parser_checkpoint.add_argument(
        "--seq_norm",
        action="store_true",
...
def main(argv):
    ...
    seq_list = [
        (str(Path(x).stem), str(Path(args.path_dataset) / x))
        for (_, x) in seq_list
    ]
